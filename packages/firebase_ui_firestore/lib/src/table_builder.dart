...
class _FirestoreTableState extends State<FirestoreDataTable> {
  ...
  Query _unwrapQuery(Query query) {
    return query;
  }
...
_FormState _initialFormStateOfValue(Object? value) {
  if (value == null) {
    ...
    return _TimestampFormState(value.toDateTime());
...
class _Source extends DataTableSource {
  ...
  @override
  int get rowCount {
    if (_aggregateSnapshot != null) return _aggregateSnapshot!;
  ...
  void onSelectAll(bool? selected) {
    ...
    if (selected) {
      _selectedRowIds.addAll(_previousSnapshot!.docs.map((e) => e.ref.id));
    } else {
  ...
  void onDeleteSelectedItems() {
    for (final doc in _previousSnapshot!.docs) {
      if (_selectedRowIds.contains(doc.ref.id)) {
        doc.ref.delete().then<void>(
              (value) => _selectedRowIds.remove(doc.ref.id),
              onError: getOnError(),
            );
      }
    }
...
class _ValueView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (value == null) {
      ...
      return Text(value.toDateTime().toString());
    } else if (value is DocumentReference) {
